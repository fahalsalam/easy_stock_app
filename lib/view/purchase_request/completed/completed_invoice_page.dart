...
class _CompletedInvoicePageState extends State<CompletedInvoicePage> {
                ...
                children: [
                  if (page == 0) ...[
                    ...
                    pw.Row(
                        ...
                        crossAxisAlignment: pw.CrossAxisAlignment.start,
                        children: [
                          pw.Text('${widget.state},${widget.country}',
                              // 'DUBAI- UAE',
                              style: pw.TextStyle(
                                  fontSize: 18,
                                  fontWeight: pw.FontWeight.bold)),
                      ...
                      ...items.asMap().entries.map((entry) {
                        ...
                        return pw.TableRow(
                          children: [
                            ...
                            // pw.Padding(
                            //   padding: const pw.EdgeInsets.all(8.0),
                            //   child: pw.Text(double.parse(item.vat)
                            //       .toStringAsFixed(2)),
                            // ),
                            pw.Padding(
                              padding: const pw.EdgeInsets.all(8.0),
  ...
  for (int i = 0; i < widget.dataList.length; i++) {
    ...
    //   bytes += generator.text(
    //     '$leftMargin${item[index].padRight(qtyWidth)}'
...
// pw.Column(
  ...
  // bytes += generator.text(
  //   '$leftMargin${widget.header.printHeader}',
  //   styles: const PosStyles(height: PosTextSize.size2, bold: true),
  // );
  // bytes += generator.text( centerText('$leftMargin${widget.header.state}, ${widget.header.country}'));
  // bytes += generator.text(centerText('$leftMargin${widget.header.trnNo}'));
  // bytes += generator.text(centerText('$leftMargin${'-' * (totalWidth - leftMarginSpaces)}')); // Divider line
